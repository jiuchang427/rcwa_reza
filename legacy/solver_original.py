...
def initialize_params(wavelengths = [1550.0],
  ...
  # Define the `params` dictionary.
    ...
    lam0 = params['nanometers'] * torch.tensor(wavelengths, dtype = torch.float32)
...
# permittivity of substrate
    ...
    params['ur2'] = 1.0# permeability in reflection region
    ...
    params['Lx'] = Lx * params['nanometers']
    params['Ly'] = Ly * params['nanometers']
...
def generate_coupled_rectangular_resonators(r_x, r_y, params,wavelength_number):
    ...
    y_mesh, x_mesh = torch.meshgrid(ya,xa)
    # Convert to tensors and expand and tile to match the simulation shape.
...
def generate_arbitrary_epsilon(eps_r, params):
    ...
    ER_t = torch.clamp(torch.real(eps_r), min=params['eps_min'], max=np.real(params['eps_max']))
    ...
    ER_t = ER_t.to(torch.complex64)
    UR_t = UR.clone().detach().to(torch.float32)
    UR_t = UR_t.to(torch.complex64)
    ER_t = ER_t.to("cuda")
    UR_t = UR_t.to("cuda")
    return ER_t, UR_t
...
# Extract commonly used parameters from the `params` dictionary.
    ...
    kinc_x0 = torch.tensor(n1 * torch.sin(params['theta']) * torch.cos(params['phi'])
                           , dtype = torch.cfloat).clone().detach().to(torch.cfloat).to("cuda")
    ...
    KZref = torch.matmul(torch.conj(params['ur1'] * I), torch.conj(params['er1'] * I))
    KZref = KZref - torch.matmul(KX, KX) - torch.matmul(KY, KY)
    KZref = torch.sqrt(KZref).to("cuda")
    KZref = -torch.conj(KZref).to("cuda")
    
    KZtrn = torch.matmul(torch.conj(params['ur2'] * I), torch.conj(params['er2'] * I))
...
#     print("-LAM",LAM.device)
    ...
    Q_ref_01 = params['ur1'] * params['er1'] * I - torch.matmul(KX, KX)
    Q_ref_10 = torch.matmul(KY, KY) - params['ur1'] * params['er1'] * I
    ...
    Q_trn_01 = params['ur2'] * params['er2'] * I - torch.matmul(KX, KX)
    Q_trn_10 = torch.matmul(KY, KY) - params['ur2'] * params['er2'] * I
...
#     print("kinc_pol",kinc_pol)
    ...
    pte_cuda = params['pte'].to("cuda")
    ptm_cuda = params['ptm'].to("cuda")
    ...
    R2 = rx2 + ry2 + rz2
    R = torch.real(-KZref / params['ur1']) / torch.real(kinc_z0 / params['ur1'])
    ...
    T2 = tx2 + ty2 + tz2
    T = torch.real(KZtrn / params['ur2']) / torch.real(kinc_z0 / params['ur2'])
    T = torch.matmul(T, T2)
