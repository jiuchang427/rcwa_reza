...
def convmat(A, P, Q):
    ...
    A = torch.fft.fftshift(torch.fft.fftn(A, dim=(4, 5)), dim=(4, 5)) / (Nx * Ny)
...
def redheffer_star_product(SA, SB):
    ...
    I = torch.eye(dim, dtype=torch.cfloat).to("cuda")
    I = I.unsqueeze(0).unsqueeze(0).unsqueeze(0)
    I = I.repeat(batchSize, pixelsX, pixelsY, 1, 1)
    
    # Calculate S11.
    S11 = torch.inverse(I - torch.matmul(SB['S11'], SA['S22'])).to("cuda")
    S11 = torch.matmul(S11, SB['S11']).to("cuda")
    S11 = torch.matmul(SA['S12'], S11).to("cuda")
    S11 = SA['S11'] + torch.matmul(S11, SA['S21'])
    
    # Calculate S12.
    S12 = torch.inverse(I - torch.matmul(SB['S11'], SA['S22'])).to("cuda")
    S12 = torch.matmul(S12, SB['S12']).to("cuda")
    S12 = torch.matmul(SA['S12'], S12).to("cuda")
    
    # Calculate S21.
    S21 = torch.inverse(I - torch.matmul(SA['S22'], SB['S11'])).to("cuda")
    S21 = torch.matmul(S21, SA['S21']).to("cuda")
    S21 = torch.matmul(SB['S21'], S21).to("cuda")
    
    # Calculate S22.
    S22 = torch.inverse(I - torch.matmul(SA['S22'], SB['S11'])).to("cuda")
    S22 = torch.matmul(S22, SA['S22']).to("cuda")
    S22 = torch.matmul(SB['S21'], S22).to("cuda")
